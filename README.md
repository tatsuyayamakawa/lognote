...
**NEXT_PUBLIC_SITE_URL**（カスタムドメインを使用する場合のみ）
```
https://yourdomain.com
```

> **カスタムドメインを使用する場合**: Search Consoleの連携には、カスタムドメインを明示的に指定する必要があります。`https://`を含め、末尾のスラッシュは不要です。Search Consoleに登録したURLと完全に一致させてください。
...
2. 以下の点をチェック：
   ...
   - JSON認証ファイルのパスが正しいか（ローカル）
   - `GOOGLE_SERVICE_ACCOUNT_JSON`環境変数が正しく設定されているか（Vercel）
   - GA4 Property IDが正しいか
   - APIが有効化されているか
   - カスタムドメインを使用している場合、`NEXT_PUBLIC_SITE_URL`が設定されているか
...
4. Vercelの場合、環境変数を追加/変更した後は必ず再デプロイが必要です

5. **Search Consoleのデータが表示されない場合**：
   - データは2〜3日の遅延があります
   - サイトがSearch Consoleに登録されているか確認
   - サービスアカウントに「オーナー」または「フル」権限が付与されているか確認
   - `NEXT_PUBLIC_SITE_URL`がSearch Consoleに登録したURLと完全一致しているか確認

6. それでも解決しない場合は、このREADMEの設定手順を最初から確認してください
